...
if (isset($GLOBALS['app_head_included']))
{
    $headinclude .= '
    <style type="text/css">
...
function get_scheme_url()
{
    ...
    $scheme_url = $baseUrl. (!empty($vbulletin->userinfo['userid']) ? '?user_id='.$vbulletin->userinfo['userid'].'&' : '?') . 'location='.$location.(!empty($other_info_str) ? '&'.$other_info_str : '');
