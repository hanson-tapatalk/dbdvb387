<?php
class vBActivity_Type_likesreceived extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		";
		$hook_query_where 	= " AND varname = 'likes' AND receiveduserid = " . $user['userid'];
		$multiplier 		= 1;
		...
		*/
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				SELECT likes_received AS $typename
