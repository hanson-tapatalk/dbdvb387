<?php
class vBActivity_Type_thanksreceived extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		";
		$hook_query_where 	= " AND varname = 'thanks' AND receiveduserid = " . $user['userid'];
		$multiplier 		= 1;
		...
		*/
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				SELECT thanks_received AS $typename
