<?php
class vBActivity_Type_thanksgiven extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		";
		$hook_query_where 	= " AND varname = 'thanks' AND dbtech_thanks_entry.userid = " . $user['userid'];
		$multiplier 		= 1;
		...
		*/
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				SELECT thanks_given AS $typename
