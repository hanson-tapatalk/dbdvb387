...
class THANKS_CACHE
{
	...
	* @public	string
	*/	
	public static $prefix 			= 'dbtech_thanks_';
	...
	*/	
	protected static $_tables		= array(
		'button' 			=> 'ORDER BY `displayorder` ASC',
	);
	...
	*/
	public static function build($type)
	{
		...
		if (!is_object(THANKS::$db))
		{
			// Ensure this doesn't error on upgrades
			return false;
		}

		THANKS::$db->hideErrors();
		$data = THANKS::$db->fetchAllKeyed('
			SELECT :dbtype.*
			FROM $:dbtype AS :dbtype
			:clauses
		', (isset(self::$_idColumns[$type]) ? self::$_idColumns[$type] : $type . 'id'), array(
			':dbtype' => $dbtype,
		...
		// Set the data
		THANKS::$cache[$field_short] = $data;
		
		foreach ((array)THANKS::$cache[$field_short] as $id => $arr)
		{
			foreach ((array)THANKS::$unserialize[$field_short] as $key)
			{
				// Do unserialize
				THANKS::$cache[$field_short][$id][$key] = @unserialize(stripslashes($arr[$key]));
			}
		}
	}
