...
)))
{
	// Invalid post id
	THANKS::outputXML(array(
		'error' => $vbphrase['dbtech_thanks_invalid_usernoteid'] . ': ' . $contentid
	));
}
...
$excluded = THANKS::refreshAjaxPost($contentid, 'usernote');
...
list($entries, $actions) = THANKS::processDisplay($noticeforum, $excluded, $post, array('dbtech_thanks_disabledbuttons' => 0), 'usernote');
