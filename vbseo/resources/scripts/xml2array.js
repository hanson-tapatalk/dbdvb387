...
var tb_not_whitespace = new RegExp(/[^\s]/);//This can be given inside the funciton - I made it a global variable to make the scipt a little bit faster.
//Process the xml data
function tb_xml2array(xmlDoc,tb_parent_count) {
	var arr;
	var parent = "";
	tb_parent_count = tb_parent_count || new Object;
	...
	*/
	...
	if(xmlDoc.parentNode && xmlDoc.parentNode.nodeName && value) {
		if(tb_not_whitespace.test(value)) {//If its a child
			arr = new Object;
	...
	if(xmlDoc.childNodes.length) {
		if(xmlDoc.childNodes.length == 1) { //Just one item in this tag.
			arr = tb_xml2array(xmlDoc.childNodes[0],tb_parent_count); //:RECURSION:
			...
			for(var i=0; i<xmlDoc.childNodes.length; i++) {//Go thru all the child nodes.
				var temp = tb_xml2array(xmlDoc.childNodes[i],tb_parent_count); //:RECURSION:
				if(temp) {
					var assoc = false;
					var arr_count = 0;
					for(key in temp) {
					...
					if(assoc && arr_count == 1) {
						if(arr[key]) { 	//If another element exists with the same tag name before,
							...
							if(!tb_parent_count || !tb_parent_count[key]) {
								tb_parent_count[key] = 0;
							...
							tb_parent_count[key]++;
							arr[key][tb_parent_count[key]] = temp[key]; //Members of of a numeric array
						} else {
							tb_parent_count[key] = 0;
							arr[key] = temp[key];
